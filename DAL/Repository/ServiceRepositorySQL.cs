    ...
    public class ServiceRepositorySQL : IServiceRepository
    {
        ...
        public int ChooseChef()
        {
            ...
            var result = db.Orders
                            ...
                            .Join(db.Chefs, i => i.Chef_FK, c => c.Chef_ID, (i, c) => new { i.Chef_FK })
                            .ToList();
            var chefs = db.Chefs.Select(i => new ChefChoose { Amount = 0, Chef_ID = i.Chef_ID }).ToList();
        ...
        {
            Restaurant db = new Restaurant();
            var result = db.Categories.Where(i => i.Category_ID == category)
                                    .Join(db.Dishes, cat => cat.Category_ID, di => di.Category_FK, (cat, di) => new DishSelection
                                    {
            ...
            {
                var ingr = db.IngredientStrings.Where(i => i.Dish_FK == dish.Dish_ID)
                                              .Join(db.Ingredients, inst => inst.Ingredient_FK, i => i.Ingredient_ID, (inst, i) => new { gramm = inst.IngredientString_Grammers, name = i.Ingredient_Name })
                                              .ToList();
        ...
        {
            Restaurant db = new Restaurant();
            var result = db.Orders.Where(i => i.Chef_FK == chef_id && (i.Status_FK == 2 || i.Status_FK == 4))
                                 ...
                                 .Join(db.DishOrders, i => i.Order_ID, d => d.Order_FK, (i, d) => new { d.DIshOrder_Id, i.Order_ID, i.Order_Number, i.Order_Date, i.Status_FK, i.Chef_FK, i.Status, d.Number, d.Dish_FK, d.Ready })
                                 .Join(db.Dishes, i => i.Dish_FK, d => d.Dish_ID, (i, d) => new ChefOrder
                                 {
